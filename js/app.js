...
function marcador(p) {
  ...
  const lineas = p.nombre.split(' ').map((w, i) => `<tspan x="1.5" dy="${i ? .3 : 0}">${w}</tspan>`).join('');
...
function imagen(src, alt, p, attrs = {}) {
  const img = el('img', { src, alt, loading: 'lazy', width: 600, height: 800, ...attrs });
...
function botonWA(p, caja, texto, etiqueta) {
  const a = el('a', {
    ...
    onclick(e) {
      ...
      e.currentTarget.href = wa(mensajeWA(p, t, caja.dataset.opcion));
    },
  });
  a.innerHTML = iconoWA;
  a.append(texto);
...
function tarjeta(p) {
  ...
  const foto = el('button', {
    type: 'button', class: 'tarjeta-foto', 'aria-label': `Ver detalle de ${p.nombre}`,
    onclick: () => abrirDetalle(p.id, foto),
  ...
  const titulo = el('h3', {}, el('button', { type: 'button', class: 'enlace', onclick: () => abrirDetalle(p.id, titulo.firstChild) }, p.nombre));
  art.append(foto, titulo, bloquePrecio(p));
  if (!agotado(p)) {
...
function abrirDetalle(id, desde) {
  ...
  const caja = el('div', { class: 'detalle-caja', 'data-id': id });
  ...
  const pieFoto = el('p', { class: 'galeria-pie' }, fotos[0].tipo);
  const miniaturas = el('div', { class: 'miniaturas', role: 'group', 'aria-label': 'Fotos' },
    ...fotos.map((f, i) => el('button', {
      ...
      onclick(e) {
        principal.onerror = () => { principal.onerror = null; principal.src = marcador(p); };
        principal.src = f.src; principal.alt = f.alt; pieFoto.textContent = f.tipo;
  ...
  const medidas = partes(p).map(pid => porId[pid]).filter(x => x.medidas).map(x => el('table', { class: 'medidas' },
    el('caption', {}, partes(p).length > 1 ? x.nombre : 'Medidas'),
  ...
  if (agotado(p)) info.append(el('p', { class: 'etiqueta etiqueta-agotado' }, 'Agotado'));
  else {
    info.append(el('h3', {}, 'Talle'), selectorTalles(p, caja), el('p', { class: 'aviso', role: 'status', hidden: true }, 'Elegí un talle para consultar'), botonWA(p, caja, 'Consultar por WhatsApp'));
  }
  if (combina.length) info.append(
    el('h3', {}, 'Combinalo con'),
    el('ul', { class: 'combina' }, ...combina.map(c => el('li', {}, el('button', {
      type: 'button', class: 'enlace', onclick: () => abrirDetalle(c.id, disparador),
    }, `${c.nombre} · ${precio(c.precio)}`)))),
  );
